<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            メッセージツール
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-10xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="max-w-screen-2xl px-4 md:px-8 mx-auto">
                        <div class="w-1/12">
                            <a href="{{ route('message.index') }}" class="text-sm block rounded bg-gray-500 p-2 mt-2 text-white text-center">一覧に戻る</a>
                        </div>

                        <div class="w-12/12 mt-5">
                            <h2 class="text-gray-800 text-2xl lg:text-3xl font-bold mb-4 md:mb-6">{{$records[0]->Subject}}</h2>
                            <p class="text-indigo-500 lg:text-lg font-semibold mb-1 md:mb-1">From : {{$records[0]->Sender}}</p>
                            </p>
                        </div>


                        <div class="flex">
                            <div class="w-5/12 shrink-0">
                                @foreach ($records as $record)
                                @php
                                $class = "";
                                if($current->id == $record->id){
                                $class = "bg-gray-100";
                                }
                                @endphp
                                <div class="mt-5 max-w-screen-md text-gray-500 md:text-lg mb-5 px-2 py-2 {{$class}}">
                                    <p class="text-indigo-500 lg:text-lg font-semibold mb-1 md:mb-1">Sent : {{$record->ReceiveDate}}</p>
                                    {!! $record->Text !!}
                                </div>
                                @endforeach
                            </div>
                            <div class="w-5/12 shrink-0 px-5 py-5">
                                @if (session('status'))
                                <div class="flex items-center bg-blue-500 text-white text-sm font-bold px-4 py-3" role="alert">
                                    <svg class="fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                        <path d="M12.432 0c1.34 0 2.01.912 2.01 1.957 0 1.305-1.164 2.512-2.679 2.512-1.269 0-2.009-.75-1.974-1.99C9.789 1.436 10.67 0 12.432 0zM8.309 20c-1.058 0-1.833-.652-1.093-3.524l1.214-5.092c.211-.814.246-1.141 0-1.141-.317 0-1.689.562-2.502 1.117l-.528-.88c2.572-2.186 5.531-3.467 6.801-3.467 1.057 0 1.233 1.273.705 3.23l-1.391 5.352c-.246.945-.141 1.271.106 1.271.317 0 1.357-.392 2.379-1.207l.6.814C12.098 19.02 9.365 20 8.309 20z" />
                                    </svg>
                                    <p>{{ session('status') }}</p>
                                </div>
                                @endif
                                <form method="POST" action="{{ route('message.send'); }}">
                                    <div>
                                        <div>
                                            @csrf
                                            {{Form::textarea('return', '', ['class' => 'form-control block w-full', 'id' => 'return', 'rows' => '10'])}}
                                            {{Form::hidden('current', $current->id)}}
                                            {{Form::hidden('itemID', $current->ItemID)}}
                                            {{Form::hidden('sender', $current->Sender)}}
                                            {{Form::hidden('parent', $current->ExternalMessageID)}}
                                        </div>
                                    </div>
                                    <x-button class="ml-4 mt-4">
                                        {{ __('返信') }}
                                    </x-button>
                                </form>
                            </div>
                            @if($item!==null)
                            <div class="w-1/12 shrink-0 py-5">
                                @if($item->image!=='')
                                <img src="{{$item->image}}" alt="" class="block" style="max-width:100%;height:auto;">
                                @endif
                                <a href="{{ route('ebay.show',['id'=>$item->id]) }}" class="text-sm block rounded bg-gray-500 p-2 mt-2 text-white text-center">EATS:{{$item->id}}</a>
                            </div>
                            @endif
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

</x-app-layout>